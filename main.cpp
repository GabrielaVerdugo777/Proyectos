...
bool puedeCom(char tab[8][8], int f, int c, int turno) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        // las fichas normales solo comen hacia adelante, la Dama ('O'/'X') no tiene restriccion
        if (ficha == 'o' && df != -2) {
            continue;
        }
        if (ficha == 'x' && df != 2) {
...
void jugar() {
    ...
    while (activo) {
        ...
        if (fDest < 0 || fDest > 7 || cDest < 0 || cDest > 7) {
            ...
            // Si estaba en cadena de captura y se sale del tablero en una dirección, le permitimos reintentar con otra dirección para q no se haga bucle
            continue;
        }
        ...
        if (rosas == 0) {
            ...
            if (totalPartidas < 100) {
                hist[totalPartidas] = "Partida " + to_string(totalPartidas + 1) + ": gana el jugador 2";
                totalPartidas++;
            }
            activo = false;
            break;
        ...
        if (verdes == 0) {
            ...
            if (totalPartidas < 100) {
                hist[totalPartidas] = "Partida " + to_string(totalPartidas + 1) + ": gana el jugador 1";
                totalPartidas++;
            }
            activo = false;
            break;
        ...
        if (rendirse == 's') {
            if (turno == 1) {
                ...
                if (totalPartidas < 100) {
                    hist[totalPartidas] = "Partida " + to_string(totalPartidas + 1) + ": gana el jugador 2 (rendicion)";
                    totalPartidas++;
                }
            } else {
                cout << "\nEl Jugador 2 se rindio, gana el jugador 1\n";
                if (totalPartidas < 100) {
                    hist[totalPartidas] = "Partida " + to_string(totalPartidas + 1) + ": Gana el Jugador 1 (rendicion)";
                    totalPartidas++;
                }
            }
            activo = false;
        ...
        if (!tieneMov(tab, turno)) {
            ...
            if (totalPartidas < 100) {
                if (turno == 1) {
                    hist[totalPartidas] = "Partida " + to_string(totalPartidas + 1) + ": Gana el Jugador 2 (Jugador 1 acorralado)";
                } else {
                    hist[totalPartidas] = "Partida " + to_string(totalPartidas + 1) + ": Gana el Jugador 1 (Jugador 2 acorralado)";
                }
                totalPartidas++;
            }
            activo = false;
